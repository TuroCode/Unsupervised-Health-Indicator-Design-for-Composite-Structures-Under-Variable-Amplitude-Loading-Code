...
matrixes=np.load("Feature_matrix_test.npy")
coeffs=np.array([0.09632068,0.83594863,0.79086729,-0.28853425,-0.09189289])
fit_coeff=np.array([0.599999,0.4,0.00000001])
row_names=["60% no1","45% no2","45% no3","60% no5","var loading 2","var loading 7"]
color_arr=["tab:green","tab:red","tab:blue","tab:orange","tab:purple","tab:brown"]
...
#Plotting of HI on all test files, and plet them then per case scenario on subfigures
...
##Compare 45% no2 with components that make it up
...
for i,feature in enumerate(matrixes):
    ...
    plt.plot(range(len(feature)),feature,linewidth="1",color=color_arr[i],alpha=0.4,label=legend_arr[i])
...
"""
#Compare original column values vs HI:
plt.subplot(121)
for column in matrixes_columns:
    np.shape(column[0])
    plt.plot(range(len(column[0])),column[0])
plt.xlabel("Hits")
